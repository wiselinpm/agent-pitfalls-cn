"""Fast Round 4 merge — hash-based dedupe + token-shingle similarity.

比 merge_all_dedup.py 快 100x：
- URL fingerprint hash (O(1))
- Title SHA1 hash (O(1))
- 标题 token-set Jaccard 用倒排索引（O(title_len) 而非 O(n²)）
- 仅在 token-set 接近时计算 SequenceMatcher
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--report", default="dedup_report_round4.json")
    p.add_argument("--sim-threshold", type=float, default=0.85)
    args = p.parse_args(argv)

    print(f"loading candidates from: {args.in_dirs}", flush=True)
    candidates: list[tuple[Path, str, dict]] = []
    for d in args.in_dirs:
        in_dir = (ROOT / d).resolve()
        if not in_dir.exists():
            print(f"  skip (missing): {in_dir}", flush=True)
            continue
        cnt = 0
    ...
    # === 1. 收集现有 pitfalls 的 token 反向索引（仅 token 数 ≥ 3 才有意义）===
    print(f"\nbuilding token inverted index...", flush=True)
    token_index: dict[str, set[int]] = defaultdict(set)  # token -> set of existing idx
    existing_titles: list[tuple[int, str]] = []  # (idx, title)
    ...
    # title_hashes: hash -> (path, fm)
    # body_hashes: hash -> (path, fm)
    ...
    for i, (path, fp, fm) in enumerate(candidates):
        ...
        # 加入 token 索引（让后续 candidate 也能匹配到它）
        for t in cand_tokens:
            if len(t) >= 2:
    ...
    if args.apply:
        ...
        report_path.write_text(
            json.dumps({"removed": removed[:1000], "kept": n_kept,
