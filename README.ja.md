...
- 🌐 **言語とプラットフォームの境界を超える** — Claude Code、OpenAI Agents SDK、LangChain、Cursor、Aiderなど14のプラットフォーム、12以上の言語、5,500以上の実例を一箇所に
...
- **自動化はスケーラビリティの必要条件** — 100個のコレクターが24時間稼働 + 3次元重複排除 + LLM分類 + 人間スポットチェックがあって初めて、このプロジェクトは5,500件規模に到達できた
- **中国語コミュニティも記録される価値がある** — 知乎、掘金、CSDN、博客園には、英語圏には見えていない豊富な落とし穴経験が眠っている

### 🛤️ 既に歩んだ道

- ✅ **Round 1-6**: 21コレクター・3,486件のpitfallから、100コレクター・5,561件の構造化ケースへと成長
