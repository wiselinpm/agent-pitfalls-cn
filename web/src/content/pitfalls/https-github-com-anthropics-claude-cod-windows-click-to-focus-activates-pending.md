...
When Claude Code is running with a **permission prompt pending** and the window does **not** have focus, the user's *first click* on the window — intended only to give the window focus — lands on the permission dialog and **submits an answer** (approve/reject)
