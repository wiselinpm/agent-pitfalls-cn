...
### Describe the bug We have started to experience error of type `"Item ‘rs_ABCD’ of type ‘reasoning’ was provided without its required following item"` when an agent does a handoff to another agent (`o4-mini` with `low` effort to `o4-mini` with `medium` effort). This error hap
