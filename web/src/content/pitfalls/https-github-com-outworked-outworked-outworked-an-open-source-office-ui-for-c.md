---
title: Outworked – An Open Source Office UI for Claude Code Agents
summary: Outworked – An Open Source Office UI for Claude Code Agents
severity: critical
platforms:
- claude-code
categories: []
symptoms: []
root_causes: []
fixes: []
references:
- title: Outworked – An Open Source Office UI for Claude Code Agents
  url: https://github.com/outworked/outworked
  source: hackernews
tags:
- claude agent
discovered_at: '2026-03-23'
verified: false
---

- [Outworked – An Open Source Office UI for Claude Code Agents](https://github.com/outworked/outworked) — hackernews

## 摘要

Outworked – An Open Source Office UI for Claude Code Agents

_来源热度：44_
