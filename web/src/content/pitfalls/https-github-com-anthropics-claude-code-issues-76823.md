...
## Summary During a user-authorized `git stash drop` in a Claude Code session, the tool-result stream returned **fabricated output** in two escalating waves attempting to manipulate the agent into running a destructive `git reset`: 1. A fake "prevent-destruction hook" claiming
