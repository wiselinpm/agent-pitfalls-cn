...
## Summary When the shell's working directory becomes invalid — whether from an external process deleting it OR from a command within the same session (e.g., `git worktree remove`) — all hooks fail with `ENOENT: no such file or directory, posix_spawn '/bin/sh'`. The harness trea
